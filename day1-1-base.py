...
# 4. 环境变量：是操作需要使用的变量
# path：当使用命令行去直接访问的程序
# 可以通过软件安装直接将path配置到了环境变量中
# 有的需要自己去配置环境变量

# 修改环境变量
# （1）可以通过图形界面：右键我的电脑，属性，高级系统设置----永久性的
# （2）通过命令行：-----是临时性，当控制台被关闭之后，就失效了
# set path :能够查看到path下的所有值
# set path=路径 可以修改path值
# %变量%
# 变量值中，不同值使用;分隔。
...
# 在python中分为两种注释
#1. 单行注释
# 从# 开始，一直该行的行尾都是注释的内容。被注释的内容是不经过python解释器
# 习惯上，注释都是放在程序的上方
# 注册的功能
def a():
    pass
...
#-*-coding:编码名称-*-
#-*-coding:utf-8-*-
# python2默认的编码集是ascii：所以在python2的py文件中必须要注明编码集，否则中文会乱码
# python3默认的编码集是utf-8：所以在python3的py文件中不需要特殊标注编码集
...
（2）不能跟关键字重名
（3）标识符是区分大小写的
（4）python3中，标识符的字符集习惯上unicode字符集  ,但是 @  $ * 一般不采用作为标识符
"""
